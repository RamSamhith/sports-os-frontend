...
// Returns the verified { email, name, picture } — throws on any
// invalid/unverified token so the controller can map to a 401.
...
const findOrCreateSocialUser = async ({ email, name, provider }) => {
...
const googleLogin = async (idToken) => {
  ...
  return findOrCreateSocialUser({ email, name, provider: 'google' });
};
