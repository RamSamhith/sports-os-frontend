...
function CompareCard({ slot, onRemove }: { slot: Entity; onRemove: () => void }) {
  ...
  const sublabel = kind === 'academy' ? `${(entity as Academy).location?.city ?? ''}, ${(entity as Academy).location?.state ?? ''}` : kind === 'coach' ? `${(entity as Coach).location?.city ?? ''} · ${(entity as Coach).experienceYears ?? 0}+ yrs` : (entity as Sport).category;
